...
class CardDetectActivity : AppCompatActivity(), ImageAnalysis.Analyzer, SurfaceHolder.Callback {
    companion object {
        private const val TAG = "CardDetectActivity"
    ...
    var cardDetectCount = 0
    var nidaDetect:Int = 0
    var headDetect:Int = 0
    var ninDetect:Int = 0
    var photoDetect:Int = 0
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (allPermissionsGranted()) {
            ...
            ActivityCompat.requestPermissions(this, CardDetectActivity.REQUIRED_PERMISSIONS, CardDetectActivity.REQUEST_CODE_PERMISSIONS)
        }
    ...
    private fun allPermissionsGranted() = CardDetectActivity.REQUIRED_PERMISSIONS.all {
        ContextCompat.checkSelfPermission(baseContext, it) == PackageManager.PERMISSION_GRANTED
    }

    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<String>, grantResults: IntArray) {
        if (requestCode == CardDetectActivity.REQUEST_CODE_PERMISSIONS) {
            if (allPermissionsGranted()) {
                startCamera()
            } else {
    ...
    private fun startCamera() {
        ...
        cameraProviderFuture.addListener(Runnable {
            ...
            try {
                ...
                Log.e(CardDetectActivity.TAG, "Use case binding failed", exc)
            }
    ...
    private fun drawDetection(
        ...
        if (cardDetectCount > 9) {
            //progressTextView.setText("Capturing ...")
            Log.d(TAG, "card detector: " + cardDetectCount)
            if (detectedPath.isNullOrEmpty()){
                ...
                if (detectedBitmap != null){
                    ...
                    //toastAlert("Save Image Successfully ...")
                }
    ...
    fun sendResultBack(path: String) {
        Log.d(CardDetectActivity.TAG, "has to return path $path")
        val intent = Intent()
    ...
    fun DrawFocusRect() {
        ...
        val  rectF = boxGenerator(previewView.width, previewView.height, 0.9)
    ...
    private fun notificationView() {
        ...
        if (cardDetectCount in 1..2) {
            msg = "PLEASE WAIT"
        } else if (cardDetectCount in 3..8) {
            msg = "PROCESSING ..."
        } else if (cardDetectCount >= 9) {
            msg = "CAPTURING ..."
        }
        progressTextView.setText(msg)
    }

    fun detectCounter(label: String, score: Float){
        if (label=="nida" && score > 0.40) {
            nidaDetect++
        } else if (label=="photo" && score > 0.25) {
            photoDetect++
        } else if (label == "head" && score > 0.25) {
            headDetect++
        } else if (label == "nin" && score > 0.20) {
            ninDetect++
        } else {
            cardDetectCount = 0
        }

        if (nidaDetect > 5 && photoDetect > 5 && headDetect > 3 && ninDetect > 2) {
            cardDetectCount++
        }
    }

    fun  toastAlert(message: String, duration: Int = Toast.LENGTH_SHORT) {
        runOnUiThread(Runnable {
    ...
    //cpp functions
    external fun initDetector(assetManager: AssetManager?): Long
    external fun destroyDetector(ptr: Long)
    external fun detect(detectorAddr: Long, srcAddr: ByteArray, width: Int, height: Int, rotation: Int): FloatArray
}
